...
"""Valida distribuciones ya creadas para formar parte de un catálogo
"""
...
def add_dataset(catalog_id, dataset_meta, datasets_input_dir,
                datasets_output_dir,
                replace=None, debug_mode=None,
    ...
    dataset_dir = os.path.join(datasets_output_dir, dataset_meta["identifier"])
...
def generate_summary_indicators(report_datasets, report_distributions):
...
def main(catalog_json_path, datasets_input_dir, datasets_output_dir,
         replace=False, debug_mode=False):

    catalog = DataJson(catalog_json_path)

    # valida cada dataset y lo copia en el catálogo de destino
    all_report_datasets = []
    ...
    for dataset in catalog.datasets:
        ...
        if any(distributions_urls):
            report_datasets, report_distributions = add_dataset(
                "sspm", dataset, datasets_input_dir, datasets_output_dir,
                replace=replace, debug_mode=debug_mode)
...
if __name__ == '__main__':
    if len(sys.argv) >= 5 and sys.argv[4]:
        replace = True if sys.argv[4] == "replace" else False
